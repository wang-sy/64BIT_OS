...
void doPrint(struct position * curPos,const int backColor, const int fontColor, const char* charFormat){
    ...
    for(row = curPos->XPosition; row < curPos->XPosition + curPos->XCharSize; row ++){
        for(col = curPos->YPosition; col < curPos->YPosition + curPos->YCharSize; col ++){
            int* pltAddr = curPos->FB_addr + row * curPos->YResolution + col; // 当前方块需要覆盖像素块的缓冲区地址
            int groupId =  (row * curPos->XCharSize + col) / 8; //  算出来在第几个char中
            int memberNum = (row * curPos->XCharSize + col) - groupId * 8; // 算出来在该char中是第几位
            char isFont = charFormat[groupId] & (1 << memberNum); // 判断该位是否为1
...
void doBackspace(struct position * curPos){
    ...
    doPrint(curPos, 0xffffff00, 0xffffff00, defaultFill);
